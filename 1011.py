'''
    ...
    Lang:
        Python 3
    Explanation:
        거리가 어떤 임의의 수의 제곱인 경우, 이동과정이 "121", "12321", "1234321"과 같은 형태로 나타나게 된다(단, 3이하의 거리는 예외적으로 거리가 장치작동횟수와 동일하다.)
        그리고 거리가 어떤 임의의 수의 제곱인 경우를 기준으로 나눠서 보면 각 구간에 있는 이동횟수의 경우의 수가 "(임의의 수)*2-1" 또는 "(임의의 수)*2-2"가 된다는 것을 알 수 있다
        (예를 들어, 거리가 25이하 17이상인 구간 안에는 이동횟수가 "5*2-1"(9) 또는 "5*2-2"(8)인 경우가 있다.)
'''
